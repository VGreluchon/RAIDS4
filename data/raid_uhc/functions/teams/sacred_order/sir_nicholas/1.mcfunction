...
execute if entity @s[tag=raid.shield_stick,scores={raid.cooldown2=..119,raid.spell=1..}] if data entity @s SelectedItem.tag.raid_uhc.shield_stick run scoreboard players set @s raid.spell 0
...
execute if entity @s[tag=raid.shield_stick,scores={raid.cooldown2=120..}] run title @s actionbar ["",{"text":"Gift number: ","color":"#0F6FD8"},{"score":{"name":"@s","objective":"raid.stat"},"color":"gold"},{"text":" cadeaux","color":"#0F6FD8"},{"text":"|Shield Stick: ","color":"#0F6FD8"},{"text":"Ready !","color":"#0F6FD8"}]
