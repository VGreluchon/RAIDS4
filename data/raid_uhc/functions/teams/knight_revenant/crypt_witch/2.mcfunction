tellraw @s ["",{"text":"Salut, ","color":"aqua"},{"selector":"@a[scores={raid.knightsrevenant=1}]","color":"gold"},{"text":"\n"},{"text":"Voici la liste des pouvoirs qu'il te reste à débloquer (clique sur le nom du pouvoir pour l'acheter):","color":"aqua"}]
...
execute if entity @s[tag=!raid.skeleton] run tellraw @s ["",{"text":"\u25b6 Skeleton:","color":"aqua","clickEvent":{"action":"run_command","value":"trigger raid.trigger4 set 1"}},{"text":" Cet arc possède un carquois contenant un nombre incalculable de flèches","color":"gold"}]
